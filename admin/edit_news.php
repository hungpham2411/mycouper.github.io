<?php
session_start();
$banner1 = $pdo->query("SELECT * FROM news WHERE ne_Id=".$_GET['id']."");
$banner1 = $banner1->fetch();

if(isset($_GET['error']) && $_GET['error']=="out_of_range"){$error="Kích thước ảnh không được quá 5MB !";}else{$error="Sửa thông tin cho tin tức";}
if(isset($_GET['error']) && $_GET['error'] == "invalid_file_format"){$error = "Ảnh không đúng định dạng !";}  else{$error = "Sửa thông tin cho tin tức";}
...
<form id="frmNEWS" name="frmNEWS" action="xulisuaNEWS.php" method="POST" enctype="multipart/form-data">
<div class="Group_Left">
<input id="id" name="id" type="hidden" placeholder="Your Id here !" value="<?php echo $banner1['ne_Id'] ?>">
<div class="dynamiclabel">
                           <input id="title" name="title" type="text" placeholder="Tiêu đề tin tức !" value="<?php echo $banner1['ne_Title'] ?>">  <a style="color:white;">(*)</a>
                        </div>
    <div class="dynamiclabel">
                         <input id="description" name="description" type="text" placeholder="Mô tả ngắn cho tin tức !" value="<?php echo $banner1['ne_Description'] ?>" />  <a style="color:white;">(*)</a>
                        </div>
<div class="dynamiclabel" style="color:white">
       Ảnh đại diện: <input type="file" id="picture" name="picture" onchange="readURL(this);"/>
<input type="hidden" id="hpc" name="hpc" value="<?php echo $banner1['ne_Picture'] ?>" />
    </div>
<div class="pre_img">
                             <span>
                                 <center><img class="prw_img" src="../public/images/blog/<?php echo  $banner1['ne_Picture']  ?>"  alt="No Image" width="95%" height="122"></center>
                             </span>
                         </div>
<script type="text/javascript">
...
<div class="Group_Right">
  <div class="dynamiclabel">
                           <input id="meta_title" name="meta_title" type="text" placeholder="Meta Title cho tin tức !" value="<?php echo $banner1['ne_MetaTitle'] ?>">  <a style="color:white;">(*)</a>
                        </div>
  <div class="dynamiclabel">
                           <textarea id="meta_description" maxlength="160" name="meta_description"  placeholder="Meta description cho tin tức !"><?php echo $banner1['ne_MetaDescription'] ?></textarea><a style="color:white;">(*)</a>
                        </div>
  <div class="dynamiclabel">
                           <textarea id="meta_keywords" maxlength="160" name="meta_keywords"  placeholder="Meta keywords cho tin tức !"><?php echo $banner1['ne_MetaKeywords'] ?></textarea><a style="color:white;">(*)</a>
                        </div>
<div class="dynamiclabel" style="color: white;"> Hiển thị: 
        <input type="radio" id="rdo" name="rdo" value="1" <?php if($banner1['ne_IsShow'] == "1"){echo "checked";} ?>  />Có &nbsp;&nbsp;
        <input type="radio" id="rdo" name="rdo" value="0" <?php if($banner1['ne_IsShow'] == "0"){echo "checked";} ?>  />Không
  <a style="color:white;">(*)</a>
    </div>
</div>
<div class="Group_FCK">Nội dung: 
   <textarea id="content" name="content" class="ckeditor" placeholder="Nội dung tin tức !"><?php echo $banner1['ne_Content'] ?></textarea>
</div>
<div class="Bao_btn">
