...
session_start();
 ...
 if(isset($_GET['error']) && $_GET['error'] == "out_of_range"){$error = "Kích thước ảnh không được quá 5MB !";} else{$error = "Thêm hỏi đáp mới";}
if(isset($_GET['error']) && $_GET['error'] == "invalid_file_format"){$error = "Ảnh không đúng định dạng !";}  else{$error = "Thêm hỏi đáp mới";}
if(isset($_GET['error']) && $_GET['error'] == "no_image"){$error = "Mời chọn ảnh cho tin tức !";}  else{$error = "Thêm hỏi đáp mới";}
?>
...
<form id="frmNEWS" name="frmNEWS" action="xulithemQA.php" method="POST" enctype="multipart/form-data">
<div class="Group_Left">
<div class="dynamiclabel">
                           <input id="title" name="title" type="text" placeholder="Tiêu đề hỏi đáp !">  <a style="color:white;">(*)</a>
                        </div>
    <div class="dynamiclabel">
                           <input id="description" name="description" type="text" placeholder="Mô tả ngắn cho hỏi đáp !">  <a style="color:white;">(*)</a>
                        </div>
...
<div class="Group_Right">
  <div class="dynamiclabel">
                           <input id="meta_title" name="meta_title" type="text" placeholder="Meta Title cho hỏi đáp !">  <a style="color:white;">(*)</a>
                        </div>
  <div class="dynamiclabel">
                           <textarea id="meta_description" maxlength="160" name="meta_description"  placeholder="Meta description cho hỏi đáp !"></textarea><a style="color:white;">(*)</a>
                        </div>
  <div class="dynamiclabel">
                           <textarea id="meta_keywords" maxlength="160" name="meta_keywords"  placeholder="Meta keywords cho hỏi đáp !"></textarea><a style="color:white;">(*)</a>
                        </div>
...
<div class="Group_FCK">Nội dung : 
    <textarea id="content" name="content" class="ckeditor" placeholder="Nội dung hỏi đáp !"></textarea>
</div>
<div class="Bao_btn">
    <input class="btn1" id="Button1" name="Button1" type="button" value="Thêm hỏi đáp" onclick="return check();" />
