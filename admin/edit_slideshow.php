<?php
$slide = $pdo->query("SELECT * FROM slideshow");
$slide = $slide->fetch();

 if(isset($_GET['error']) && $_GET['error'] == "out_of_range"){$error = "Kích thước ảnh không được quá 5MB !";} else{$error = "Sửa thông tin Banner ";}
if(isset($_GET['error']) && $_GET['error'] == "invalid_file_format"){$error = "Ảnh không đúng định dạng !";}  else{$error = "Sửa thông tin Banner ";}
if(isset($_GET['action']) && $_GET['action'] == "success"){echo "<script>alert('Cập nhật thông tin Banner thành công !')</script>";}
?>
...
<div class="dynamiclabel" style="color: white;"> Hiển thị: 
        <input type="radio" id="rdo" name="rdo" value="1" <?php if($slide['ss_IsShow'] == 1){echo "checked";} ?> />Có &nbsp;&nbsp;
        <input type="radio" id="rdo" name="rdo" value="0" <?php if($slide['ss_IsShow'] == 0){echo "checked";} ?> />Không &nbsp;&nbsp;
  <a style="color:white;">(*)</a>
	...
	<div class="dynamiclabel">
						...
						<div class="dynamiclabel">
                           <input id="register" name="register" type="text" placeholder="Đường dẫn đăng ký !" value="<?php echo $slide['ss_LinkRegister'] ?>">  <a style="color:white;">(*)</a>
                        </div>
</div>
<div class="Bao_btn">
